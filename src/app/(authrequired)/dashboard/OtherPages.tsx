...
export default function OtherPages() {
    ...
    return <div className="flex mt-[10px] w-full justify-end  py-[30px] items-center gap-[4px] max-h-[30px]">
        ...
        {pages.map(page => (
            <Button key={page.number} onClick={() => setSelectedPage(page.number)} className={cn(' hover:-translate-y-2 transition-transform dark:bg-black dark:text-white h-[38px] w-[35px]  bg-white hover:bg-black/10 text-black', { " hover:bg-black/80 text-white  bg-[#1D1DCE]": selectedPage === page.number })} >{page.number}</Button>
        ))}
