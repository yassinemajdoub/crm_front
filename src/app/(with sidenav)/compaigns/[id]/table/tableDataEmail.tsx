...
export function TableDataEmail<TData, TValue>({
  columns,
  data,
  params
}: DataTableProps<TData, TValue>) {
  const [clickedButton, setClickedButton] = useState(null);
  const buttons = ['All', 'Sent', 'Scheduled'];
 ...
 <div className="flex justify-between">
      <div className="flex h-10 gap-1 w-[300px]">
        {buttons.map((label, index) => (
            <button
              key={index}
              onClick={() => handleClick(index)}
              className={`justify-center items-center px-2.5 py-2 flex min-w-[55px] h-9 rounded-tl border border-gray-200 rounded-tr
                ${
                  clickedButton === index
            ...
            >
              <div className="text-zinc-800 text-[13px] leading-[13px]">
                {label}
              </div>
            </button>
          ))}
      ...
      {originalDataList.length > 0 && (
        <div className="flex justify-end items-center w-[1100px] py-4 px-5">
          <button>
          <TrashListView                                     
                className=""
