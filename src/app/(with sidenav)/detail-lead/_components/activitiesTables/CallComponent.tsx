...
import {  Call,fetchCallsByLeadId } from '../../utils/fetchActivites';
...
export default function CallComponent({params}:CallLeadProps) {
    ...
    const fetchCalls = async () => {
      try {
        const { data, error } = await fetchCallsByLeadId({ id: params.id });
        if (error) {
      ...
      <div className="absolute top-0 right-0 h-full w-[721px] shadow-lg bg-white p-28">
        <section className='flex flex-col'>
          <div className='flex justify-between items-center'>
            <div className="w-[274px] text-black text-xl font-medium leading-[25px]">Add Call</div>
